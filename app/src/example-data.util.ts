...
import type { ForceGraphData } from "./components/forcegraph";
import { evaluations, projects, } from "./example-data-raw";
...
export function mapExampleDataToForceGraph(
  ...
  function spreadAttribute(attr: "feasibility" | "viability", margin: number) {
    if (nodes.length <= 1) {
    ...
    // Map original scores linearly into [margin, 1-margin] while preserving relative distances
    ...
    for (let k = 1; k < positions.length; k++) {
      const need = positions[k - 1] + minSep;
      if (positions[k] < need) positions[k] = need;
    }

    // Backward pass: pull back-to-fit within upper bound if forward pass pushed things out of range
...
export const exampleDataMapped = mapExampleDataToForceGraph(
  projects,
  evaluations
);
