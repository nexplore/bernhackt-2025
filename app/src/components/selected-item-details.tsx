...
export function SelectedItemDetails({ selectedNode, allNodes, allLinks }: SelectedItemDetailsProps) {
  if (!selectedNode) {
    return (
      <SidebarGroup>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="space-y-1 text-sm">
              <div>Feasibility: <Badge variant="secondary">{formatNumber(selectedNode.feasibility)}</Badge></div>
              <div>Viability: <Badge variant="secondary">{formatNumber(selectedNode.viability)}</Badge></div>
              <div>Benefit: <Badge 
                variant={selectedNode.benefit > 0.1 ? "default" : selectedNode.benefit < -0.1 ? "destructive" : "secondary"}
              >
                {formatNumber(selectedNode.benefit)}
              </Badge></div>
              <div>Size: <Badge variant="outline">{selectedNode.size}</Badge></div>
            </div>
          </div>
